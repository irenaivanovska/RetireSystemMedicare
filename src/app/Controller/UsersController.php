...
class UsersController extends AppController {
      ...
      if ($this->request->is('post')) {
        ...
        if (!empty($postData['User']['user_id'])){
          ...
          if (!empty($postData['User']['email'])){
            $this->set_token($postData,$sess_id);
          }
  ...
  public function login($auto=null) {
    if ($this->request->is('post')) {
      if ($this->Auth->login()) {
        $messages = '';
        if (!$this->Favorites->moveFromSession('Plans.Favorites', $this->loadModel('FavoritePlans'), 'plan_id', 'user_id')) {
          $messages .= '<br /> Unable to save favorite plans.';
        }
        if (!$this->Favorites->moveFromSession('Drugs.Favorites', $this->loadModel('FavoriteDrugs'), 'drug_id', 'user_id')) {
          $messages .= '<br /> Unable to save favorite drugs.';
        } 
         
        //$this->redirect($this->Auth->redirect());
        if($auto===true) {
          $this->redirect(array('controller' => 'users', 'action' => 'member_type'));
        }
        else {
