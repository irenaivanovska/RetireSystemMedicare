...
class FaqCatController extends AppController {
  ...
  public function beforeFilter() {
    parent::beforeFilter();
    $this->Auth->allow('index', 'getEntries', 'find_faq', 'faqCats', 'findTerms', 'view');
  }
  ...
  public function faqCats($role=NULL) {
    ...
    foreach($all as $eB) {
      ...
      if($numEnt==0 && $role!='admin') {
        continue;
      }
      $act=false;
      if($this->FaqCat->id==$faqID) {
        $act=true;
      }
      $eFaq['active']=$act;
  ...
  function searchFAQ($postData) {
    ...
    if(!empty($faqSearch)) {
      ...
      foreach($faqSearch as $eRes) {
        ...
        $ans=$item['faq_answer'];
        foreach($stw as $eT) {
          if(stripos($ans,$eT)!==false || stripos($ques,$eT)!==false) {
            $f++;
          } // finds how many words matched
        }
        if(stripos($ans,$searchTerm)!==false || stripos($ques,$searchTerm)!==false) {
          $f+=5;
        } // add bonus points
